...
def save_short_answer_corpus():
    ...
    save_as_pickle(short_answer_corpora, os.path.join(config.SHARED_CONTAINER, 'model', 'word_embedding', 'corpora'),
                   'short_answer_corpora.pkl')


def get_tagged_document_corpora():
    save_essay_corpus()
    essay_corpus = get_model_from_pickle(os.path.join(config.SHARED_CONTAINER, 'model', 'word_embedding', 'corpora'),
                                         'essay_corpora.pkl')
    ...
    if config.ESSAY_ONLY:
        ...
        short_answer_corpus = get_model_from_pickle(
            os.path.join(config.SHARED_CONTAINER, 'model', 'word_embedding', 'corpora'), 'short_answer_corpora.pkl')
